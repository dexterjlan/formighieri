...
async function loadGestaoMarceneirosList() {
    ...
    marceneiros.forEach(marceneiro => {
        const tr = document.createElement('tr');
        tr.dataset.cabinetMakerId = String(marceneiro.id);
        tr.innerHTML = `
            <td class="p-3">
...
async function saveGestaoMarceneiroRow(row) {
    ...
    const cabinetMakerId = Number(row.dataset.cabinetMakerId);
...
async function deleteGestaoMarceneiroRow(row) {
    ...
    const { count, error: countError } = await supabaseClient
        .from('OrderProject')
        .select('id', { count: 'exact', head: true })
        .eq('cabinetMakerId', cabinetMakerId);

    if (countError) {
        if (countError.message?.includes('cabinetMakerId')) {
